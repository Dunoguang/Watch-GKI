...
void susfs_add_sus_path_loop(void __user **user_info) {
	...
	(void)strscpy(new_list->info.target_pathname, info.target_pathname, SUSFS_MAX_LEN_PATHNAME - 1);
	(void)strscpy(new_list->target_pathname, info.target_pathname, SUSFS_MAX_LEN_PATHNAME - 1);
...
void susfs_add_sus_kstat(void __user **user_info) {
	...
	hash_for_each_possible_safe(SUS_KSTAT_HLIST, tmp_entry, tmp_hlist_node, node, info.target_ino) {
		if (!strcmp(tmp_entry->info.target_pathname, info.target_pathname)) {
			...
			SUSFS_LOGI("is_fuse: %d, is_statically: '%d', target_ino: '%lu', target_pathname: '%s', spoofed_ino: '%lu', spoofed_dev: '%lu', spoofed_nlink: '%u', spoofed_size: '%lld', spoofed_atime_tv_sec: '%ld', spoofed_mtime_tv_sec: '%ld', spoofed_ctime_tv_sec: '%ld', spoofed_atime_tv_nsec: '%lu', spoofed_mtime_tv_nsec: '%lu', spoofed_ctime_tv_nsec: '%lu', spoofed_blksize: '%ld', spoofed_blocks: '%lld', is successfully added to SUS_KSTAT_HLIST\n",
					new_entry->is_fuse,
...
void susfs_set_uname(void __user **user_info) {
	...
	if (!strcmp(info.release, "default")) {
		(void)strscpy(my_uname.release, utsname()->release, __NEW_UTS_LEN);
	} else {
		(void)strscpy(my_uname.release, info.release, __NEW_UTS_LEN);
	}
	if (!strcmp(info.version, "default")) {
		(void)strscpy(my_uname.version, utsname()->version, __NEW_UTS_LEN);
	} else {
		(void)strscpy(my_uname.version, info.version, __NEW_UTS_LEN);
	}
...
void susfs_spoof_uname(struct new_utsname* tmp) {
	...
	do {
		...
		(void)strscpy(tmp->release, my_uname.release, __NEW_UTS_LEN);
		(void)strscpy(tmp->version, my_uname.version, __NEW_UTS_LEN);
...
void susfs_set_cmdline_or_bootconfig(void __user **user_info) {
	...
	(void)strscpy(fake_cmdline_or_bootconfig,
...
void susfs_add_open_redirect(void __user **user_info) {
	...
	(void)strscpy(new_entry_redirected->info.target_pathname, info.redirected_pathname, SUSFS_MAX_LEN_PATHNAME - 1);
	(void)strscpy(new_entry_redirected->info.redirected_pathname, info.target_pathname, SUSFS_MAX_LEN_PATHNAME - 1);
...
int susfs_open_redirect_spoof_do_proc_readlink(struct inode *inode, char *tmp_buf, int buflen) {
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node, inode->i_ino) {
			...
			entry->target_dev == inode->i_sb->s_dev)
		{
			...
			(void)strscpy(tmp_buf, entry->info.redirected_pathname, SUSFS_MAX_LEN_PATHNAME - 1);
			srcu_read_unlock(&susfs_srcu_open_redirect, srcu_idx);
			return 0;
		}
	}
...
void susfs_show_variant(void __user **user_info) {
	...
	(void)strscpy(info.susfs_variant, SUSFS_VARIANT, SUSFS_MAX_VARIANT_BUFSIZE-1);
	info.err = 0;
out_copy_to_user:
...
void susfs_show_version(void __user **user_info) {
	...
	(void)strscpy(info.susfs_version, SUSFS_VERSION, SUSFS_MAX_VERSION_BUFSIZE-1);
	info.err = 0;
out_copy_to_user:
