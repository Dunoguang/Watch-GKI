			 ...
			 void *data)
{
	...
	int i;
	bool new_sb;
	bool is_cgroup2 = fs_type == &cgroup2_fs_type;
	unsigned long magic = is_cgroup2 ? CGROUP2_SUPER_MAGIC :
					   CGROUP_SUPER_MAGIC;
...
static bool is_cgroupfs_sb(struct super_block *sb)
{
	return sb->s_type == &cgroup_fs_type || sb->s_type == &cgroup2_fs_type;
}
...
static u64 cgroup_freeze_read(struct cgroup_subsys_state *css,
			      struct cftype *cft)
{
	return 0;
}

static int cgroup_freeze_write(struct cgroup_subsys_state *css,
			       struct cftype *cft, u64 val)
{
	return val <= 1 ? 0 : -EINVAL;
}
...
int cgroupstats_build(struct cgroupstats *stats, struct dentry *dentry)
{
	...
	if (!is_cgroupfs_sb(dentry->d_sb) || !kn || kernfs_type(kn) != KERNFS_DIR)
		return -EINVAL;
...
static struct cftype cgroup_dfl_base_files[] = {
	...
	},
	{
		.name = "cgroup.freeze",
		.flags = CFTYPE_NOT_ON_ROOT,
		.read_u64 = cgroup_freeze_read,
		.write_u64 = cgroup_freeze_write,
	},
	{ }	/* terminate */
};
...
/**
 * cgroup_get_from_fd - get a cgroup pointer from a cgroup directory fd
 * @fd: fd obtained by opening a cgroup2 directory
 */
